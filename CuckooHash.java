...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        ...
        int position = hash1(key);  // Start with hash1 position
        int iterations = 0;         // Track displacements to detect cycles

        while (iterations < CAPACITY) {
            if (table[position] == null) {
                // If the current bucket is empty, place the newBucket here and return
                table[position] = newBucket;
                return;
            }

            // If this key-value pair is already in the table, do nothing and return
            if (table[position].getBucKey().equals(key) && table[position].getValue().equals(value)) {
                return;
            }

            // Kick out the current bucket and place the new one in its place
            Bucket<K, V> kickedOutBucket = table[position];
            table[position] = newBucket;

            // Prepare to move the kicked-out bucket to its alternate position
            newBucket = kickedOutBucket;
            position = (position == hash1(newBucket.getBucKey())) ? hash2(newBucket.getBucKey()) : hash1(newBucket.getBucKey());

            iterations++;
        }

        // If a cycle is detected, rehash and try again
        rehash();
        put(key, value);
    }
