...
class BloomFilter {
    ...
    public boolean contains(String s) {
        for (int n = 0; n < noHashes; n++) {
            long hc = hashCode(s, n); // get nth hash code
            int bitNo = (int) (hc & this.hashMask); // get valid index

            // if no bits are set there is no element in the list
            if (!data.get(bitNo)) {
                return false;
            }
        }
        // if all bits are set the element is in the set
        return true;
    }
